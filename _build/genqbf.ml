...
| Forall_gen of variable list
| Exists_gen of variable list
...
(*          | _ -> [] *)
(*  *)
(* let get_model_set desc= *)
(*    let varinit = get_vars desc.init in *)
(*    let vartrans = get_vars desc.trans in *)
(*    two_varlists_to_set varinit vartrans *)
(*  *)
(* (* not completed *) *)
(* let encode_formula desc k= *)
(*    let rec encode rest i = *)
(*        match rest with *)
(*        | Form.Exists(x,y) -> let new_desc = generate_unrolled_desc desc k x in *)
(*                         let model_set = get_model_set new_desc in *)
(*  *)
(*        | Form.Forall(x,y) -> let new_desc = generate_unrolled_desc desc k x in *)
(*                         let model_set = get_model_set new_desc in *)
(*  *)
(*        | Form.Globally(e)     -> unroll_expr_uptok_G_P e k *)
(*        | Form.Finally(e)     -> unroll_expr_uptok_F_P e k *)
(*        | Form.Next(e)     -> unroll_expr_X_P e k *)
(*        | Form.Until(e1,e2)   -> (match sema with *)
(*                            | Args.OPT -> unroll_expr_uptok_OPT_U_P e1 e2 k (* optimistic *) *)
(*                            | Args.PES -> unroll_expr_uptok_PES_U_P e1 e2 k) (* pessimistic *) *)
(*        | Form.Release(e1,e2)   -> (match sema with *)
(*                            | Args.OPT -> unroll_expr_uptok_OPT_R_P e1 e2 k (* optimistic *) *)
(*                            | Args.PES -> unroll_expr_uptok_PES_R_P e1 e2 k) (* pessimistic *) *)
(*        | Form.Not(x)        -> Not(encode x i) *)
(*        | Form.Or(x,y)       -> Or(encode x i, encode y i) *)
(*        | Form.And(x,y)      -> And(encode x i, encode y i) *)
(*        | Form.Impl(x,y) -> Impl(encode x i, encode y i) *)
(*        | Form.Equiv(x,y)     -> Equiv(encode x i, encode y i) *)
(*    in *)
...
let _ =
  ...
  try
    ...
    let _ = pr_time "parsing Model and HyperCTL* files" in
